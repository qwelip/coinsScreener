...
export const getSortedPercentGrowCandlesStat = (data: ICoinCandlesStat[], candlesToCheck: number, sortingDirection: SortDirection): ICoinCandlesStat[] => {
  const res = data.map(item => {
    const firstCandleToCheck = Number(item.result.list.at(candlesToCheck - 1)![4])
    const secondCandleToCheck = Number(item.result.list.at(0)![4])
